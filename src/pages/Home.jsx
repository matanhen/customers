...
export default function Home() {
  ...
  const assetsLiabilitiesData = [
    { name: 'נכסים', value: totalInvestments },
    { name: 'התחייבויות', value: totalDebts },
  ];
      ...
      <div className="relative mb-10 rounded-3xl overflow-hidden">
        ...
        <div className="relative px-8 py-12 lg:py-16 text-center">
          ...
          <p className="text-lg text-white/80 max-w-xl mx-auto mb-6">
            כל הנתונים הפיננסיים שלך במקום אחד
          </p>

          <Button
            onClick={() => {
              // open the floating advisor
              const btn = document.querySelector('[title="יועץ פיננסי AI"]');
              if (btn) btn.click();
            }}
            className="bg-[#c8a863] hover:bg-[#d4b87a] text-[#105330] font-bold px-8 py-3 rounded-2xl text-base shadow-xl"
          >
        ...
        <Card className="border-0 shadow-lg bg-gradient-to-br from-blue-50 to-white">
          <CardContent className="p-5">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xs text-gray-500">שווי השקעות</p>
                <p className="text-lg font-bold text-blue-700">₪{totalInvestments.toLocaleString()}</p>
              </div>
            </div>
          </CardContent>
        </Card>
            ...
            <CardContent>
              <ResponsiveContainer width="100%" height={240}>
                <BarChart data={incomeVsExpensesData} barGap={4}>
                  ...
                  <Bar dataKey="הוצאות" fill="#c8a863" radius={[4,4,0,0]} />
                </BarChart>
              </ResponsiveContainer>
            </CardContent>
          </Card>
            ...
            <CardContent>
              <div className="grid grid-cols-3 gap-6 items-center">
                <div className="text-center p-6 rounded-2xl bg-emerald-50">
                  ...
                  <p className="text-3xl font-black text-emerald-700">₪{totalInvestments.toLocaleString()}</p>
                </div>
                <div className={`text-center p-6 rounded-2xl ${netWorth >= 0 ? 'bg-purple-50' : 'bg-orange-50'}`}>
                  <p className="text-sm text-gray-500 mb-1">שווי נקי</p>
                  <p className={`text-3xl font-black ${netWorth >= 0 ? 'text-purple-700' : 'text-orange-600'}`}>
                    ₪{netWorth.toLocaleString()}
                  </p>
                </div>
                <div className="text-center p-6 rounded-2xl bg-red-50">
                  <p className="text-sm text-gray-500 mb-1">סה"כ חובות</p>
                  <p className="text-3xl font-black text-red-600">₪{totalDebts.toLocaleString()}</p>
                </div>
              </div>
            </CardContent>
          </Card>
