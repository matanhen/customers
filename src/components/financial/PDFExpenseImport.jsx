...
הוצאות קבועות (fixed): ${FIXED_EXPENSE_CATEGORIES.join(', ')}

יתרת הוצאות (variable): ${VARIABLE_EXPENSE_CATEGORIES.join(', ')}

אם הוצאה לא מתאימה לאף קטגוריה ברשימה, סמן אותה כ-custom.
אל תכלול הכנסות, רק הוצאות.
אל תכלול עסקאות של העברות בין חשבונות.
...
החזר JSON תקין בלבד.`,
      ...
      const items = (result.expenses || []).map(exp => ({
        ...
        amount: exp.amount,
        type: exp.expense_type === 'custom' ? 'variable' : exp.expense_type,
        category: exp.expense_type === 'custom' ? '' : (exp.category || ''),
        isCustom: exp.expense_type === 'custom',
        customName: exp.expense_type === 'custom' ? (exp.custom_name || exp.description || '') : '',
        confirmed: true,
      }));
  ...
  const handleApply = () => {
    const validItems = parsedItems.filter(item =>
      item.confirmed &&
      item.amount > 0 &&
      (item.isCustom ? item.customName : item.category)
    );
    onApply(validItems);
    handleClose();
  };
  ...
  const confirmedCount = parsedItems.filter(i => i.confirmed && (i.isCustom ? i.customName : i.category)).length;
        ...
        {step === 'upload' && (
          <div className="space-y-4 py-4">
            ...
            <div className="p-3 bg-blue-50 rounded-lg text-sm text-blue-800">
              ...
              <p>ה-AI יקרא את הקובץ, יזהה את ההוצאות וישייך אותן לקטגוריות. תוכל לעבור על הפילוח ולשנות לפני האישור הסופי.</p>
            </div>
          </div>
        )}
        ...
        {step === 'review' && (
          <div className="space-y-4 py-2">
            <div className="p-3 bg-emerald-50 rounded-lg text-sm text-emerald-800">
              <p className="font-semibold">נמצאו {parsedItems.length} הוצאות. עבור על הפילוח ואשר:</p>
            </div>
                ...
                >
                  <div className="flex items-start justify-between mb-3">
                    ...
                    <div className="flex items-center gap-2">
                      {item.confirmed && (item.isCustom ? item.customName : item.category) && (
                      ...
                      <div>
                        ...
                        >
                          ...
                          <SelectContent className="max-h-[200px]">
                            {(item.type === 'fixed' ? FIXED_EXPENSE_CATEGORIES : VARIABLE_EXPENSE_CATEGORIES).map(cat => (
                              <SelectItem key={cat} value={cat}>{cat}</SelectItem>
                            ))}
                            <SelectItem value="custom">➕ הוצאה שלא ברשימה</SelectItem>
                          </SelectContent>
                        </Select>
                      </div>
                      {item.isCustom && (
                        <div className="col-span-2">
                          <Label className="text-xs mb-1 text-slate-500">שם ההוצאה</Label>
                          <Input
