...
import { Loader2, Upload, X, ImagePlus, PanelTop } from 'lucide-react';
import { Card, CardContent } from '@/components/ui/card';
import { Button } from '@/components/ui/button';

export default function NavbarLogoUploader() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const recs = await base44.entities.SiteSettings.filter({ key: 'navbar_logo' });
        if (recs && recs[0]) {
          setLogoUrl(recs[0].logo_url || '');
          setRecordId(recs[0].id);
        }
      } catch (e) {
        console.error('Failed to load navbar logo', e);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleFileChange = async (e) => {
    ...
    try {
      ...
      if (recordId) {
        ...
        const created = await base44.entities.SiteSettings.create({ key: 'navbar_logo', logo_url: url });
      ...
      <CardContent className="p-6">
        <div className="flex items-center gap-4 mb-4">
          <div className="p-3 rounded-2xl bg-gradient-to-br from-[#105330] to-[#1a7a4a] shadow-lg shadow-emerald-500/30">
            <PanelTop className="w-6 h-6 text-white" />
          </div>
          <div>
            <h3 className="text-lg font-bold text-slate-800">לוגו הסרגל העליון</h3>
            <p className="text-sm text-slate-500">תמונה שתופיע בסרגל העליון במקום הטקסט "צעירים מתעשרים" (מומלץ רקע שקוף/לבן)</p>
          </div>
        </div>
        {loading ? (
          ...
          <div className="flex flex-col md:flex-row md:items-center gap-4">
            {logoUrl ? (
              <div className="w-48 h-20 rounded-2xl bg-[#105330] flex items-center justify-center p-2">
                <img
                  src={logoUrl}
                  alt="לוגו סרגל"
