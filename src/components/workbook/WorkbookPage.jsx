...
• להרוויח – להרוויח בכל חודש יותר באופן עקבי.
• להגן – לשלם פחות מיסים למדינה באופן חוקי ולהשאיר לעצמך יותר כסף בפועל.
• לנהל – לנהל את הכסף בצורה מושלמת ולחסוך את המקסימום האפשרי
• למנף – לייצר מהכסף יותר כסף בעזרת השקעות שמייצרות הכנסות פסיביות לעבר חופש כלכלי
...
דרג את הציונים שלך בכל אחד ממרכיבי מעגל החופש: מ-1 (לא טוב) עד 10 (מושלם!)`,
    questions: [
      ...
      { key: 'score_protect', label: 'להגן (ציון היכולת שלך לשלם פחות מיסים באופן חוקי ולהשאיר לעצמך יותר כסף) מ-1 עד 10', type: 'rating' },
...
export default function WorkbookPage({ userId, viewerEmail }) {
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-4" dir="rtl">
      ...
      {/* Save Button */}
      <div className="flex items-center justify-between bg-white border border-[#c8a863] rounded-2xl px-5 py-3 shadow-md">
        {savedAt ? (
        ...
        <Button
          onClick={handleSave}
          disabled={saving}
          className="bg-[#c8a863] hover:bg-[#b8943d] text-[#105330] font-bold px-6"
        >
